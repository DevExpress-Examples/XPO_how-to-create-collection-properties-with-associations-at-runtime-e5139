	...
	Friend Class Program
		Shared Sub Main(ByVal args() As String)
			...
			Using session As New UnitOfWork()
				...
				Dim s1 As New Supplier(session) With {.CompanyName = "Acme", .Address = "XY/101"}
				Dim s2 As New Supplier(session) With {.CompanyName = "Zorq", .Address = "Moonbase"}
				Dim p1 As New Product(session) With {.Name = "Anvil", .Quantity = 1}
				miSupplier.SetValue(p1, s1)
				Dim p2 As New Product(session) With {.Name = "Cow", .Quantity = 80}
				CType(miProducts.GetValue(s1), XPCollection).Add(p2)
				Dim p3 As New Product(session) With {.Name = "Saucer", .Quantity = 3}
