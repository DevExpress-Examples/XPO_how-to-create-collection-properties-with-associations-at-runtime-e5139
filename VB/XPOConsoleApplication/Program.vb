...
Namespace XPOConsoleApplication
    ...
    Friend Class Program
        ...
        Shared Sub Main(ByVal args As String())
            ...
            Dim types As Type() = New Type() {GetType(Supplier), GetType(Product), GetType(Group)}
            ...
            Using session As UnitOfWork = New UnitOfWork()
                ...
                Dim s1 As Supplier = New Supplier(session) With {.CompanyName = "Acme", .Address = "XY/101"}
                Dim s2 As Supplier = New Supplier(session) With {.CompanyName = "Zorq", .Address = "Moonbase"}
                Dim p1 As Product = New Product(session) With {.Name = "Anvil", .Quantity = 1}
                miSupplier.SetValue(p1, s1)
                Dim p2 As Product = New Product(session) With {.Name = "Cow", .Quantity = 80}
                CType(miProducts.GetValue(s1), XPCollection).Add(p2)
                Dim p3 As Product = New Product(session) With {.Name = "Saucer", .Quantity = 3}
                ciProduct.GetMember("Supplier").SetValue(p3, s2)
                Dim g1 As Group = New Group(session) With {.Title = "Flying"}
